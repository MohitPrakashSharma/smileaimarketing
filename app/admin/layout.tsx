...
const NAV_ITEMS = [
  ...
  { label: "Meetings", path: "/admin/appointments", Icon: IconCalendarCheck },
    ...
    <div className="theme-navy min-h-screen bg-background font-body text-foreground">
      ...
      <header className="sticky top-0 z-40 hidden border-b border-border bg-surface/95 backdrop-blur-md lg:block">
        <div className="mx-auto flex h-16 max-w-[1400px] items-center gap-6 px-6">
          ...
          <nav aria-label="Admin primary" className="flex items-center gap-1">
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <Link
                  key={item.path}
                  href={item.path}
                  className={`flex items-center gap-2 rounded-full px-3.5 py-2 text-body-small font-semibold transition-colors ${
                    isActive
          ...
          <form onSubmit={handleSearch} className="ml-auto hidden max-w-xs flex-1 items-center xl:flex">
            <div className="relative w-full">
              ...
              <input
                ...
                className="h-10 w-full rounded-full border border-border bg-background pl-9 pr-3 text-body-small text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary"
              />
            </div>
          </form>
          ...
          {showPrimaryAction && (
            <Link
              ...
              className="inline-flex h-10 shrink-0 items-center rounded-full bg-primary px-4 text-body-small font-bold text-primary-foreground transition-colors hover:bg-primary-hover"
            >
              + New Campaign
            </Link>
          )}
          ...
          <div className="relative shrink-0" ref={userMenuRef}>
            <button
              ...
              className="flex h-10 w-10 items-center justify-center rounded-full bg-surface-muted text-body-small font-bold text-foreground transition-colors hover:bg-primary/15"
              aria-label="Account menu"
            >
              A
            </button>
            {userMenuOpen && (
              <div className="absolute right-0 top-12 w-56 rounded-xl border border-border bg-surface p-2 shadow-lg">
      ...
      <header className="sticky top-0 z-40 flex h-14 items-center justify-between border-b border-border bg-surface/95 px-4 backdrop-blur-md lg:hidden">
        ...
        <div className="flex items-center gap-2">
          {showPrimaryAction && (
            <Link
              href="/admin/campaigns"
              className="inline-flex h-9 items-center rounded-full bg-primary px-3.5 text-metadata font-bold text-primary-foreground"
            >
              + Campaign
            </Link>
          )}
