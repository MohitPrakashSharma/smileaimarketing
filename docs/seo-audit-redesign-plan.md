...
**Remaining caveat — PageSpeed not validated against a live response.** Keyless calls hit Google's shared anonymous daily quota (HTTP 429, project 583797351490) and the existing Google key's project has the PageSpeed Insights API disabled (HTTP 403). The normalizer (`normalizePsiResponse`) is validated only against fixtures modelled on the documented v5 response. Before production: enable the PageSpeed Insights API on the Google Cloud project (or set a dedicated `PAGESPEED_API_KEY`) and run one audit to confirm field/lab/diagnostic extraction against a real response.
