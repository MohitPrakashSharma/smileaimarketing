...
const PERFORMANCE_CHECKS: CheckDefinition[] = [
  ...
  diagCheck({ id: "perf.diag.images", title: "Images are oversized, unoptimised or not lazy-loaded", auditIds: ["uses-optimized-images", "uses-responsive-images", "modern-image-formats", "offscreen-images"], weight: 5, severity: "MEDIUM", impact: 4, effort: 2, metric: "image_savings_bytes", expected: "< 100 KB of potential image savings", why: "Images are almost always the biggest thing a phone has to download. Serving them compressed, correctly sized and lazy-loaded is the cheapest large speed win there is.", fix: "Compress images, upload them at the size they're displayed, and let images below the fold load lazily.", developerFix: "Serve WebP/AVIF with `srcset`/`sizes`, add `loading=\"lazy\"` below the fold, and compress the files listed in evidence.", test: (d) => { const b = d.reduce((s, x) => s + (x.savingsBytes ?? 0), 0); return { fails: b >= 100 * 1024, detected: `${kb(b)} potential savings across ${d.map((x) => x.id.replace(/^uses-/, "")).join(", ")}` }; } }),
  diagCheck({ id: "perf.diag.lcp_resource", title: "The main (LCP) image is lazy-loaded or not prioritised", auditIds: ["lcp-lazy-loaded", "prioritize-lcp-image"], weight: 4, severity: "MEDIUM", impact: 4, effort: 1, metric: "lcp_resource", expected: "The LCP image is discoverable early, not lazy-loaded, and preloaded", why: "The hero image is the single element that decides when the page 'looks loaded'. Lazy-loading it, or letting it queue behind other files, delays that moment for every visitor.", fix: "Make sure the first big image on the page loads immediately, not lazily.", developerFix: "Remove `loading=\"lazy\"` from the LCP image, add `fetchpriority=\"high\"` and `<link rel=\"preload\" as=\"image\">` for it.", test: (d) => { const lazy = d.find((x) => x.id === "lcp-lazy-loaded"); const prio = d.find((x) => x.id === "prioritize-lcp-image"); const fails = (lazy?.score !== null && lazy?.score !== undefined && lazy.score < 1) || (prio?.savingsMs ?? 0) >= 200; return { fails, detected: [lazy && lazy.score !== null && lazy.score < 1 ? "LCP image is lazy-loaded" : null, prio?.savingsMs ? `${ms(prio.savingsMs)} saveable by prioritising the LCP image` : null].filter(Boolean).join("; ") || "ok" }; } }),
  diagCheck({ id: "perf.diag.caching", title: "Static assets served without long-lived caching", auditIds: ["uses-long-cache-ttl"], weight: 3, severity: "LOW", impact: 2, effort: 1, metric: "cache_wasted_bytes", expected: "Images, CSS and JS cached for ≥ 1 year", why: "Returning visitors and every page after the first re-download files the browser could have kept. Fixing cache headers is a server setting, not a redesign.", fix: "Ask your host/developer to enable long cache lifetimes for images, scripts and styles.", developerFix: "Send `Cache-Control: public, max-age=31536000, immutable` for versioned static assets (see files in evidence).", test: (d) => { const b = d[0].items.reduce((s, i) => s + (i.wastedBytes ?? 0), 0) || (d[0].savingsBytes ?? 0); return { fails: (d[0].score ?? 1) < 0.5 && b >= 100 * 1024, detected: `${kb(b)} of assets with short/no cache lifetime` }; } }),
  diagCheck({ id: "perf.diag.compression", title: "Text assets served without compression", auditIds: ["uses-text-compression"], weight: 3, severity: "MEDIUM", impact: 3, effort: 1, metric: "compression_savings_bytes", expected: "HTML, CSS and JS served with gzip or Brotli", why: "Without compression, text files are 3–5× larger than they need to be. It's a one-line server change.", fix: "Ask your host to enable gzip/Brotli compression.", developerFix: "Enable Brotli/gzip at the web server or CDN for text/html, text/css, application/javascript, application/json.", test: (d) => ({ fails: (d[0].savingsBytes ?? 0) >= 30 * 1024, detected: `${kb(d[0].savingsBytes)} saveable with compression` }) }),
  diagCheck({ id: "perf.diag.third_party", title: "Third-party scripts block the main thread", auditIds: ["third-party-summary"], weight: 4, severity: "MEDIUM", impact: 3, effort: 2, metric: "third_party_blocking_ms", expected: "Third-party code blocks the main thread for < 250 ms", why: "Chat widgets, tag managers, ad pixels and social embeds run on every page load and you don't control their code. Each one costs real time on a phone.", fix: "Remove third-party scripts you no longer use and load the rest after the page is interactive.", developerFix: "Defer/lazy-load third-party tags (facade for chat/video embeds), consolidate tag managers; see the per-vendor blocking time in evidence.", test: (d) => { const blocking = d[0].items.reduce((s, i) => s + (i.wastedMs ?? 0), 0); const total = blocking || (d[0].numericValue ?? 0); return { fails: total >= 250, detected: `${ms(total)} main-thread blocking from ${d[0].items.length} third part${d[0].items.length === 1 ? "y" : "ies"}` }; } }),
  ...
  diagCheck({ id: "perf.diag.fonts", title: "Web fonts block text rendering", auditIds: ["font-display"], weight: 2, severity: "LOW", impact: 2, effort: 1, metric: "font_display", expected: "All web fonts use font-display: swap/optional", why: "While a custom font downloads, text can stay invisible — the page looks blank longer than it needs to.", fix: "Ask your developer to let text show in a fallback font while the custom font loads.", developerFix: "Add `font-display: swap` (or `optional`) to @font-face rules and preload the primary font file.", test: (d) => ({ fails: (d[0].score ?? 1) < 1, detected: `${d[0].items.length} font file${d[0].items.length === 1 ? "" : "s"} without font-display` }) }),
