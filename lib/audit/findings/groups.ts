...
import { maxSeverity, priorityScore } from "../priority";
...
const GROUPS: GroupDef[] = [
  ...
  { key: "thin_duplicate_content", title: "Thin or duplicated page content", pillar: "CONTENT", section: "C3", checkIds: ["content.thin_page", "content.duplicate_body"], why: "Pages with little or copied text rarely rank. Google rewards pages that fully answer what a {customer} is looking for, in their own words.", fixIntro: "Expand thin pages and make duplicated pages genuinely different (or merge them).", owner: "owner" },
  { key: "images", title: "Images are missing SEO basics", pillar: "CONTENT", section: "C4", checkIds: ["content.images.missing_alt", "content.images.no_dimensions", "content.images.oversized"], why: "Images without alt text are invisible to search; oversized or unsized images slow the page and make it jump as it loads.", fixIntro: "Add alt text, set dimensions, and compress large photos.", owner: "owner" },
  ...
  { key: "trust_signals", title: "Trust and contact signals are incomplete", pillar: "CONTENT", section: "C6", checkIds: ["content.trust.no_contact_page", "content.trust.no_about_page", "content.trust.no_privacy_page", "content.trust.no_nap_on_home"], why: "Google's quality guidelines look for who is behind a site and how to reach them. {cap_customers} look for the same things before they call.", fixIntro: "Make sure the site has clear about, contact and privacy pages and shows your address and phone.", owner: "owner" },
  { key: "conversion_path", title: "It's hard for a visitor to take the next step", pillar: "CONTENT", section: "C7", checkIds: ["content.conversion.no_tel_link", "content.conversion.no_primary_cta", "content.conversion.no_form"], why: "Ranking only pays off if the visitor can act. A tap-to-call number, a visible '{booking}' button and a short form are the difference between traffic and {customers}.", fixIntro: "Put a clickable phone number, one clear call-to-action and a short form where visitors can see them.", owner: "owner" },
];
...
export function buildFindings(runs: CheckRun[], ctx: CheckContext): Finding[] {
  ...
  for (const g of GROUPS) {
    ...
    findings.push(assemble(g.key, g.title, g.pillar, g.section, g.why, g.fixIntro, g.owner, members, denominator, fill));
  }

  // Ungrouped failing checks become findings on their own.
  for (const r of failing) {
    if (grouped.has(r.def.id)) continue;
    findings.push(assemble(r.def.id, r.def.title, r.def.pillar, r.def.section, r.def.why, r.def.fix, "developer", [r], denominator, fill));
  }
